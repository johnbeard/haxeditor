...
bool FAL::OSDependedOpen(wxFileName& myfilename, FileAccessMode FAM, unsigned ForceBlockRW)
{
    ...
    //Owning file
    else if( not myfilename.IsFileReadable() and DoFileExists )
    { // "and myfilename.FileExist()" not used because it's for just plain files, not for block ones.
...
bool FAL::FALOpen(wxFileName& myfilename, FileAccessMode FAM,
        unsigned ForceBlockRW)
{
    ...
    {		//FileExists()){
        ...
        if (not IsOpened())
        {
            ...
            wxMessageBox(_("File cannot open."), _("Error"),
                    wxOK | wxICON_ERROR);
            return false;
        }
...
bool FAL::SetAccessMode(FileAccessMode fam)
{
    ...
    if (Access(the_file.GetFullPath(),
            (fam == ReadOnly ? wxFile::read : wxFile::read_write)))
    {
        Close();
        Open(the_file.GetFullPath(),
                (fam == ReadOnly ? wxFile::read : wxFile::read_write));
        if (!IsOpened())
        {
            wxBell();
            ...
            wxMessageBox(
                    _(
                            "File load error!.\nFile closed but not opened while access change. For avoid corruption close the program"),
    ...
    wxBell();
    wxMessageBox(
            wxString(_("File cannot open in this mode : ")).Append(
                    FAMtoString(fam)), _("Error"), wxOK | wxICON_ERROR);
    return false;
}
...
DiffNode* FAL::NewNode(uint64_t start_byte, const char* data, int64_t size,
        bool inject)
{
...
size_t FAL::BlockWrite(unsigned char* buffer, unsigned size)
{
    ...
    std::cout << "buffer write : " << size << "put_ptr :" << put_ptr
            << std::endl;
...
bool FAL::Apply(void)
{
    ...
    if (file_access_mode != ReadOnly)
        for (unsigned i = 0; i < DiffArray.GetCount(); i++)
        {
            if (DiffArray[i]->flag_commit == DiffArray[i]->flag_undo)
            {		// If there is unwriten data
                ...
                if (BlockRWSize > 0)
                {
                    uint64_t StartSector = put_ptr / BlockRWSize;
                    unsigned StartShift = put_ptr - StartSector * BlockRWSize;
                    uint64_t EndSector = (put_ptr + DiffArray[i]->size)
                            / BlockRWSize;
                    ...
                    if (ProcessID >= 0)
                    {
                        ...
                        while (rd < rd_size)
                        {
                            word = ptrace(PTRACE_PEEKTEXT, ProcessID,
                                    reinterpret_cast<char*>(StartSector
                                            * BlockRWSize + rd), NULL);
                    ...
                    memcpy(bfr + StartShift,
                            (DiffArray[i]->flag_commit ?
                                    DiffArray[i]->old_data :
                                    DiffArray[i]->new_data),
                            DiffArray[i]->size);
                    ...
                    if (ProcessID >= 0)
                    {
                        ...
                        while (wr < rd_size)
                        {
                            ...
                            if (ptrace(PTRACE_POKETEXT, ProcessID,
                                    reinterpret_cast<char*>(StartSector
                                            * BlockRWSize + wr), word) == -1)
                                wxMessageBox(
                                        _(
                                                "Error on Write operation to Process RAM"),
                                        _("FATAL ERROR"));
                            wr += 4;
                        }
                        success *= true;
                ...
                if (success)
                    DiffArray[i]->flag_commit =
                            DiffArray[i]->flag_commit ? false : true;//alter state of commit flag
            }
        }
    return success;
}
...
int64_t FAL::Undo(void)
{
    ...
    for (unsigned i = 0; i < DiffArray.GetCount(); i++)
        ...
        {		//find first undo node if available
            if (i != 0)
...
{
    std::cout << "\n\nNumber\t" << "Start\t" << "Size\t" << "DataN\t"
            << "DataO\t" << "writen\t" << "Undo\t" << "Injection\n";
    for (unsigned i = 0; i < DiffArray.GetCount(); i++)
    {
        std::cout << i << '\t' << DiffArray[i]->start_offset << '\t'
                << DiffArray[i]->size << '\t';
        if (DiffArray[i]->size > 0)
            std::cout << std::hex
                    << (DiffArray[i]->new_data == NULL ?
                            0 : (unsigned short) DiffArray[i]->new_data[0])
                    << '\t'
                    << (DiffArray[i]->old_data == NULL ?
                            0 : (unsigned short) DiffArray[i]->old_data[0])
                    << '\t' << DiffArray[i]->flag_commit << '\t'
                    << DiffArray[i]->flag_undo << '\t'
                    << DiffArray[i]->flag_inject << std::dec << std::endl;
        else
            std::cout << std::hex << 0 << '\t'
                    << (unsigned short) DiffArray[i]->old_data[0] << '\t'
                    << DiffArray[i]->flag_commit << '\t'
                    << DiffArray[i]->flag_undo << std::dec << std::endl;
    }
}
...
long FAL::ReadR(unsigned char* buffer, unsigned size, uint64_t from,
        ArrayOfNode* PatchArray, int PatchIndice)
{
#ifdef _DEBUG_FILE_
    ...
    if (PatchIndice == 0)	//Deepest layer
    {
...
#endif // __WXGTK__
        {
            ...
            if (ProcessID >= 0)
            {
                ...
                {
                    word = ptrace(PTRACE_PEEKTEXT, ProcessID,
                            reinterpret_cast<char*>(StartSector * BlockRWSize
                                    + rd), NULL);
                    memcpy(bfr + rd, &word, 4);
                    rd += 4;
                }
            ...
            memcpy(buffer, bfr + StartShift,
                    wxMin(wxMin(rd, rd_size - StartShift), size)); //wxMin protects file ends.
            delete[] bfr;
    ...
    if (PatchIndice != 0)
    {
        ...
        if (patch->flag_inject and patch->size < 0)
        { //Deletion patch
            readsize = DeletionPatcher(from, buffer, size, PatchArray,
                    PatchIndice);
        }
        else if (patch->flag_inject)
        {	//Injection patch
            readsize = InjectionPatcher(from, buffer, size, PatchArray,
                    PatchIndice);
        }
        else
        {
            ...
            if (size != readsize) //If there is free chars
                readsize =
                        (Length() - from > size) ? (size) : (Length() - from);//check for buffer overflow
...
long FAL::InjectionPatcher(uint64_t current_location, unsigned char* data,
        int size, ArrayOfNode* PatchArray, int PatchIndice)
{
    ...
    if (PatchArray->Item(PatchIndice - 1)->size > 0
            and PatchArray->Item(PatchIndice - 1)->flag_inject)
    {
        ...
        {
            return ReadR(data, size, current_location - Inject_Node->size,
                    PatchArray, PatchIndice - 1);
        }
        ///State ...(),,,[....]..... -> ...(..[xx),,],..
                ...
                and Inject_Node->end_offset() < current_location + size)
        {
            ...
            int read_size = ReadR(data + first_part, size - first_part,
                    Inject_Node->start_offset, PatchArray, PatchIndice - 1); //Than copy second part.
                ...
                and Inject_Node->end_offset() < current_location + size)
        {
            ...
            int read_size = ReadR(data, first_part, current_location,
                    PatchArray, PatchIndice - 1); //Read First Part from lower layer
            memcpy(data + first_part, Inject_Node->new_data, Inject_Node->size); // Copy second part to buffer
            read_size += ReadR(data + first_part + Inject_Node->size,
                    size - (first_part + Inject_Node->size),
                    current_location + first_part, PatchArray, PatchIndice - 1); //Read latest chunk
        ...
        {
            return ReadR(data, size, current_location, PatchArray,
                    PatchIndice - 1);
        }
    }
    return -1;
...
long FAL::DeletionPatcher(uint64_t current_location, unsigned char* data,
        int size, ArrayOfNode* PatchArray, int PatchIndice)
{
    /******* MANUAL of Code Understanding ******
     * current_location                   = [   *
    ...
    if (PatchArray->Item(PatchIndice - 1)->size < 0)
    {
        ...
        {
            return ReadR(data, size, current_location - Delete_Node->size,
                    PatchArray, PatchIndice - 1);
        }
        ///State ...[xxx(...)...],,,... -> ...[xxx()...,,,]...
        ///State ...[xxx(...]...),,,... -> ...[xxx(),,,...]...
        else if (Delete_Node->start_offset > current_location
                and Delete_Node->start_offset < current_location + size)
        {
            int first_part_size = Delete_Node->start_offset - current_location;
            int read_size = 0;
            read_size += ReadR(data, first_part_size, current_location,
                    PatchArray, PatchIndice - 1);
            read_size += ReadR(data + first_part_size, size - first_part_size,
                    current_location + first_part_size - Delete_Node->size,
                    PatchArray, PatchIndice - 1);
            return read_size;
        }
        ///State ...[...]...(...)... -> ...[...]...()...
...
void FAL::ModificationPatcher(uint64_t current_location, unsigned char* data,
        int size, DiffNode* Patch)
{
    /***** MANUAL of Code understanding *********
     * current_location                    = [   *
     * current_location + size             = ]   *
    ...
    if (Patch->flag_undo && !Patch->flag_commit)// Allready committed to disk, nothing to do here
        return;
    ///State: ...[...(xxx]xxx)...
    if (current_location <= Patch->start_offset
            && current_location + size >= Patch->start_offset)
    {
        int irq_loc = Patch->start_offset - current_location;
        ///...[...(xxx)...]... //not neccessery, this line includes this state too
        int irq_size =
                (size - irq_loc > Patch->size) ?
                        (Patch->size) : (size - irq_loc);
        memcpy(data + irq_loc,
                Patch->flag_undo ? Patch->old_data : Patch->new_data, irq_size);
    }

    ///State: ...(xxx[xxx)...]...
    else if (current_location <= Patch->start_offset + Patch->size
            && current_location + size >= Patch->start_offset + Patch->size)
    {
        int irq_skipper = current_location - Patch->start_offset;//skip this bytes from start
        int irq_size = Patch->size - irq_skipper;
        memcpy(data,
                Patch->flag_undo ?
                        Patch->old_data : Patch->new_data + irq_skipper,
                irq_size);
    }

    ///State: ...(xxx[xxx]xxx)...
    else if (Patch->start_offset <= current_location
            && Patch->start_offset + Patch->size >= current_location + size)
    {
        int irq_skipper = current_location - Patch->start_offset;//skip this bytes from start
        memcpy(data,
                Patch->flag_undo ?
                        Patch->old_data : Patch->new_data + irq_skipper, size);
    }
}

bool FAL::Add(uint64_t start_byte, const char* data, int64_t size,
        bool injection)
{
    ...
    for (unsigned i = 0; i < DiffArray.GetCount(); i++)
    {
        ...
        {
            if (DiffArray.Item(i)->flag_commit)
            {	// commited undo node
